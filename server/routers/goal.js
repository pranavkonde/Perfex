...
goalRouter.post('/createGoal', async (req, res) => {
    try {
        ...
        const { status, description, sDate, eDate, rating, priority, title,weightage } = req.body;
        ...
        res.status(500).json({ message: 'Error creating goal', error: error.toString() });
    }
});
...
goalRouter.get('/:goalId', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error retrieving goal', error: error.toString() });
    }
});
...
goalRouter.get('/emp/:employeeId', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error retrieving goals', error: error.toString() });
    }
});
